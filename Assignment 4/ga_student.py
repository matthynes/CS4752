...
# NOTE: Consult README.txt for experimentation results.
#
...
def get_ga_settings(sudoko_size):
    ...
    settings.population_size = 120  # total size of each population                             (experiment with this)
    settings.elitism_ratio = 0.4  # select top x% of individuals to survive                   (experiment with this)
    settings.parent_roulette_ratio = 0.1  # select x% of population as parents via roulette wheel     (experiment
    # with this)
...
def evolve(population, settings):
    ...
    # sorts pop_fitness and transforms fitnesses into their indices then returns the last E individuals
    elite = sorted(range(len(pop_fitness)), key=lambda x: pop_fitness[x])[-E:]
    elite = [population[e] for e in elite]
    ...
    offspring = []
    while len(offspring) < len(population) - len(next_population):
        ...
        # crossover first part of mother with second part of father
        child1 = mother[:settings.crossover_index] + father[settings.crossover_index:]
        # crossover first part of father with second part of mother
    ...
    for i in range(M):
        ...
        # make sure we don't mutate the same child twice
        ...
        offspring[i] = child
        # lists are unhashable so we have to use a tuple (performance impact?)
