...
from gamestate_solution import GameState as DisplayState

# set which version of the GameState you will use for each Player in the game
from gamestate_solution import GameState as P1GameState
from gamestate_solution import GameState as P2GameState
...
class Connect4:
    def __init__(self):
        ...
        self.player_states = [P1GameState(BOARD_ROWS, BOARD_COLS),
                              P2GameState(BOARD_ROWS, BOARD_COLS)]
    ...
    def draw_piece(self, surface, tile, color, border):
        ...
        pg.draw.circle(self.screen, BLACK,
                       (col * TILESIZE + TILESIZE // 2, row * TILESIZE + TILESIZE // 2),
                       TILESIZE // 2 - PIECEPAD + border, border)
    ...
    def do_turn(self):
        ...
        if self.winner == PLAYER_NONE:  # there is no winner yet, so get the next move from the AI
            player = self.display_state.player_to_move()  # get the next player to move from the
            #  state
    ...
    def draw(self):
        ...
        if (self.winner == PLAYER_NONE):
            self.draw_text(PLAYER_NAMES[player] + (
            ": Human" if self.players[player] == None else ": AI Thinking"), self.text_position,
                           PIECECOLOR[player])
        else:
            self.draw_text(GAME_RESULT_STRING[self.winner], self.text_position,
                           PIECECOLOR[self.winner])
        pg.display.flip()
        ...
        for event in pg.event.get():
            ...
            if event.type == pg.MOUSEBUTTONDOWN:
                if pg.mouse.get_pressed()[0]:
                    ...
                    if self.display_state.is_legal(
                            move) and self.display_state.winner() == PLAYER_NONE:
                        self.do_move(move)
